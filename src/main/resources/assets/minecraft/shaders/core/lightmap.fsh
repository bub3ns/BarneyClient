...
uniform vec3 BarneyNightTint;
uniform float BarneyNightStrength;
...
void main() {
    ...
    if (BarneyNightStrength > 0.0) {
        ...
        if (!guiCorner) {
            ...
            vec3 night = mix(BarneyNightTint, vec3(1.0), desat) * lit;
            color = mix(color, night, BarneyNightStrength);
        }
    }
