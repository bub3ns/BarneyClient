...
extends AbstractSetting {
    ...
    public Component buildComponent() {
        Component component = new Component().add(new ScrollingTextComponent(Font.REGULAR.metrics(8.0f), () -> Localization.translate(this.key)).setColorProvider(scrollingTextComponent -> ColorPalette.PRIMARY_TEXT_COLOR.mulAlpha(0.75f + 0.25f * scrollingTextComponent.hover())).fadeOut().fill()).add(new ValueFormatter(Font.REGULAR.metrics(7.0f), this::getValue, this::updateValue, this.minValue, this.maxValue).setDisplayValue(() -> (this.step < 1.0f && this.step > 0.0f ? moscow.rockstar.util.NumberFormatting.formatOneDecimal(this.getValue()) : moscow.rockstar.util.NumberFormatting.formatDecimal(this.getValue()))).setSuffix(this::formatValue).setTextColorProvider(valueFormatter -> ColorPalette.ACCENT_COLOR)).gap(6.0f).layout(Layout.ROW).overflowMode(JustifyContent.SPACE_BETWEEN).alignment(Alignment.CENTER).padding(Insets.of(6.0f, 0.0f, 0.0f, 0.0f)).fillWidth();
