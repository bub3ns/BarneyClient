...
/**
 * Clears the blocks above a one-block step before moving onto it.
 */
public final class UpwardClearanceStrategy extends ExcavationController {
    ...
    public UpwardClearanceStrategy(BlockPositionOffset start, BlockPositionOffset end) {
        super(start, end);
    }
    ...
    public boolean isValidUpwardClearance(BlockNavigationFactory navigation) {
        ...
        if (end.getY() - start.getY() != 1
            ...
            || !navigation.hasSolidSupport(end.getX(), end.getY() - 1, end.getZ())) {
            return false;
        }

        int breakTicks = 0;
        int blocksToClear = 0;
        for (BlockPos position : this.clearancePositions()) {
    ...
    public NavigationResult updateRotation() {
        ...
        if (obstruction != null) {
            ...
            Direction face = getBestBlockFace(player, obstruction);
    ...
    private BlockPos[] clearancePositions() {
        BlockPositionOffset start = BlockPositionOffset.fromBlockPosition(this.getStartPosition());
        BlockPositionOffset end = BlockPositionOffset.fromBlockPosition(this.getEndPosition());
        return new BlockPos[]{
            new BlockPos(start.getX(), start.getY() + 2, start.getZ()),
            new BlockPos(end.getX(), end.getY() + 1, end.getZ()),
