...
/** Error mapping for the Minecraft Services client API. */
public interface ClientErrorHandler<R> extends JsonResponseParser<R> {
    @Override
    default void handleErrorResponse(HttpResponse response, JsonObjectToken body) throws IOException {
        if (body.containsString("error") && body.containsString("errorMessage")) {
            throw new ClientApiException(response, body.getString("error"), body.getString("errorMessage"));
        }
        if (body.containsString("errorMessage")) {
            throw new ClientApiException(response, Integer.toString(response.getStatusCode()), body.getString("errorMessage"));
        }
    }
}
